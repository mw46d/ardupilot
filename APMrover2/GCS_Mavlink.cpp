...
uint8_t Rover::set_speed(float type, float speed, float thrttle) {
    ...
    case 1:                                         // Ground Speed
        if (speed > 0.0) {                            // in m/s
            ...
            gcs().send_text(MAV_SEVERITY_INFO, "Cruise speed: %.1f m/s", g.speed_cruise);
        }

        if (thrttle > 0.0 && thrttle <= 100.0) {
            g.throttle_cruise.set_and_save(thrttle);
            gcs().send_text(MAV_SEVERITY_INFO, "Cruise throttle: %.1f", g.throttle_cruise);
        }
